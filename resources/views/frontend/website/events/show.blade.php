                ...
                <div class="col-sm-12">
                    ...
                    <div class="event_detail pt-4 pl-5 pr-5 pb-3">
                        <div class="row">
                            <div class="col-8 d-flex">
                                ...
                                <h5 class="text-muted">{{$event->address}}</h5>
                                </div>
                            </div>
                    ...
                    <div class="event_content mt-5">
                        ...
                        <div class="col-sm-12">
                            ...
                            {{-- {{dd($event->users()->whereGender(0)->count())}} --}}
                            {{-- && $logged_in_user->profile->gender = 0  ||$women_count >= $event->limit && $logged_in_user->profile->gender == 1 --}}
                            {{-- @if( $women_count >= $event->limit && $logged_in_user->profile->gender == 1  )
                            {{'ok'}}
                            @else 
                            {{'not'}}
                            @endif --}}
                            ...
                            {{-- @if( $men_count >= $event->limit && $logged_in_user->profile->gender == 0  )
                            {{'ok'}}
                            @else 
                            {{'not'}}
                            @endif --}}
                                {{-- {{$logged_in_user->profile->gender}} --}}
                            ...
                            <div class="tickets mt-4 ">
                                @if (Auth::check() && $women_count >= $event->f_limit && $logged_in_user->profile->gender == 1)
                                <form method="post" action="{{ route('frontend.events.waitlist', $event->id) }}">
                                        @csrf
                                        @if ($event->type != 1)
                                    <div class="men_ticket row bg-grey p-3">
                                        <div class="col-lg-3  col-6">
                                            <h5 class="font-weight-bold">Men (Price {{ $event->price_male }}))</h5>
                                            
                                        </div>
                                        <div class="col-lg-3 col-6">
                                                <input type="radio" class="form-check-inline" value="0" name="registration_gender" id="men_radio" required>
                                            {{-- <h5 class="font-weight-bold">Man Ticket Quantity --}}
                                            </h5>
                                    ...
                                    @endif
                                    @if ($event->type != 2)
                                    <div class="women_ticket row bg-grey p-3">
                                        <div class="col-lg-3 col-6">
                                            <h5 class="font-weight-bold">Women (Price {{ $event->price_female }}))</h5>
                                            
                                        </div>
                                        <div class="col-lg-3 col-6">
                                                <input type="radio" class="form-check-inline" value="1" name="registration_gender" id="women_radio" required>
         
                                        </div>
                                        <div class="col-12 mt-3">
                                              
                                                <button   class="btn btn-theme btn-lg" type="submit" style="cursor:pointer">Join Wait List</button>
                                                                      
                                        </div>
                                    </div>
                                    @endif
                                </form>
                                {{-- && $logged_in_user->profile->gender == 0  --}}
                            @elseif(Auth::check() && $men_count >= $event->limit && $logged_in_user->profile->gender == 0)
                            <form method="post" action="{{ route('frontend.events.waitlist', $event->id) }}">
                                    @csrf
                                @if ($event->type != 1)
                                <div class="men_ticket row bg-grey p-3">
                            ...
                            @else
                            <form action="{{route('frontend.contact.register', $event->id)}}" method="POST">
                                    @csrf
                                    @if ($event->type != 1)
                                <div class="men_ticket row bg-grey p-3">
                                    <div class="col-lg-3  col-6">
                                    ...
                                    <div class="col-12 mt-3">
                                        ...
                                        <button type="submit" class="btn btn-theme btn-lg">Continue</button>
