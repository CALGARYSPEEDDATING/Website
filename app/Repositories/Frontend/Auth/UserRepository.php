...
class UserRepository extends BaseRepository
{
    ...
    public function create(array $data)
    {
        return DB::transaction(function () use ($data) {
            $c_dob = $data['birth_month'].'/'.$data['birth_date'].'/'.$data['birth_year'];
            // dd($data);
            $user = parent::create([
                ...
                'dob'               => $this->changeDateFormat($c_dob),
                'phone'             => $data['phone'],
                // If users require approval or needs to confirm email
                'confirmed'         => config('access.users.requires_approval') || config('access.users.confirm_email') ? 0 : 1,
            ]);
            if($data['matches_contact'] == 'Primary Email'){
                $data['matches_contact'] = $data['email'];
            }
            if($data['matches_contact'] == 'Primary Phone'){
                $data['matches_contact'] = $data['phone'];
            }
            if($data['matches_contact'] == 'Both'){
                $data['matches_contact'] = $data['email'] .'| '.$data['phone'];
            }
            if ($user) {
    ...
    public function update($id, array $input, $image = false)
    {
        ...
        $user->profile()->update([
            ...
            'matches_info' => isset($input['matches_info']) ? $input['matches_info'] : '',
            'subscribed' => isset($input['subscribed']) ? 1 : 0,
            'show_image' => isset($input['show_image']) ? 1 : 0,
            'question_one' => $input['question_one'],
            'question_two' => $input['question_two'],
            'question_three' => $input['question_three'],
            'question_four' => $input['question_four'],
